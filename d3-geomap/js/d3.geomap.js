...
// add id to svg, by Zoe
            self.svg = selection.append('svg').attr('width', self.properties.width).attr('height', self.properties.height).attr('id','map-svg');
            ...
            d3.json(self.properties.geofile, function (error, geo) {
                ...
                self.svg.append('g').attr('class', 'units zoom').selectAll('path').data(topojson.feature(geo, geo.objects[self.properties.units]).features).enter().append('path').attr('class', function (d) {
                    ...
                    // add id to make it easier to get the element, by Zoe
                    return self.properties.unitPrefix + d.id;
                })
                // added by Zoe to highlight selected country
        ...
        value: function update() {
            ...
            self.data.forEach(function (d) {
                ...
                if (!unit.empty() && self.defined(val)) {
                    ...
                    unit.select('title').text(text + '\n\n' + self.properties.column + ': ' + val.slice(0,val.length-3));
                }
            });
        ...
        value: function drawLegend() {
            ...
            sg.selectAll('text').data(colors).enter().append('text').text(function (d, i) {
                ...
                if (i === steps - 1) {
                    ...
                    var text = self.properties.format(self.colorScale.invertExtent(d)[0]);
                    return text.slice(0,text.length-3);
                };
